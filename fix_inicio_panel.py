...
with open('src/app/coach/InicioPanel.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
states = """  const [orientacionData, setOrientacionData] = useState<any[]>([])
...
fetch_call = """        // Fetch Agenda (Last 14 days to today+1 for volume relativity)
...
old_orientacion = """<div style={{ width: '100%', height: 160, marginTop: 16 }}>
...
new_orientacion = """<div style={{ width: '100%', marginTop: 24, padding: 12 }}>
                {!loading && orientacionData.length > 0 ? (
                  <>
...
dist_ui = """      {/* Distribucion de Tareas */}
...
if "import { BarChart, Bar," in content and "PieChart," not in content:
    content = content.replace("import { BarChart, Bar,", "import { BarChart, Bar, PieChart as AnimatedPieChart, Pie, Cell,")
else:
    # They might be imported separately, let's just make sure we import them from recharts
    if "import { PieChart as AnimatedPieChart, Pie, Cell } from 'recharts'" not in content:
        content = re.sub(r"import { ([^}]*) } from 'recharts'", r"import { \1, PieChart as AnimatedPieChart, Pie, Cell } from 'recharts'", content)


with open('src/app/coach/InicioPanel.tsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Done patching InicioPanel.tsx")
