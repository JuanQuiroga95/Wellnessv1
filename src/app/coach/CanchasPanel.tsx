...
export default function CanchasPanel(){
  ...
  useEffect(()=>{
    ...
    if (document.getElementById('leaflet-css')) {
      if ((window as any).L) setLoaded(true)
      return
    }
    ...
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      <div className="anim-up" style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
        ...
        <div style={{display:'flex',gap:6}}>
          {(['buscar','guardadas'] as const).map(t=>(
            <button className="hover-scale" key={t} onClick={()=>{
              setTab(t);
      ...
      <div style={{display: tab==='buscar'?'flex':'none', flexDirection:'column', gap:16}}>
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          ...
          <button className="hover-scale" onClick={searchLocation} disabled={loading} style={{...C.btn,opacity:loading?0.5:1}}>
          ...
          <button className="hover-scale" onClick={loadPitches} disabled={loading} style={C.btnGhost} title="Buscar estadios en esta zona">
            ⚽ Buscar estadios aquí
          </button>
          <button className="hover-scale" onClick={startMeasure} style={{...C.btnGhost,color:measuring?'var(--lime)':'var(--silver)',borderColor:measuring?'var(--lime)':'var(--fog)'}} title="Medir cancha manualmente">
          ...
          <div style={{...C.card, padding:12, background:'var(--ink)', border:'1px solid var(--lime)'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:4}}>
              {locationResults.map((loc, i) => (
                <button className="hover-scale" key={i} onClick={() => selectLocation(loc)} style={{...C.btnGhost, textAlign:'left', border:'none', background:'var(--ink3)', display:'flex', justifyContent:'space-between'}}>
          ...
          <div style={{...C.card,background:'rgba(200,241,53,.06)',border:'1px solid rgba(200,241,53,.2)'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
              ...
              <div style={{display:'flex', gap:8}}>
                <button className="hover-scale" onClick={handleSaveManual} style={{...C.btn, padding:'8px 16px'}}>⭐ Guardar medición</button>
                <button className="hover-scale" onClick={startMeasure} style={C.btnGhost}>📏 Medir otra</button>
              </div>
            </div>
          </div>
        )}
        ...
        {pitches.length>0&&(
          <div style={C.card}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6,maxHeight:300,overflowY:'auto'}}>
              {pitches.map(p=>(
                <button className="hover-scale" key={p.id} onClick={()=>{
                  selectPitch(p);
        ...
        {selected&&(
          <div style={{...C.card,border:'1px solid rgba(200,241,53,.25)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:12,marginBottom:16}}>
              ...
              <button className="hover-scale" onClick={()=>selectPitch(null)} style={{background:'none',border:'none',color:'var(--fog)',cursor:'pointer',fontSize:18}}>✕</button>
            </div>
            ...
            <div style={{borderTop:'1px solid var(--mist)',paddingTop:14}}>
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'flex-end'}}>
                ...
                <button className="hover-scale" onClick={()=>savePitch(selected)} disabled={saving} style={{...C.btn,opacity:saving?0.5:1}}>{saving?'Guardando...':'⭐ Guardar'}</button>
              </div>
              ...
              {saved.map(s=>(
                <div key={s.id} style={{display:'flex',alignItems:'center',gap:12,padding:'14px 18px',background:'var(--ink3)',border:'1px solid var(--mist)',borderRadius:14,transition:'all .12s'}}>
                  ...
                  <div style={{display:'flex',gap:6,flexShrink:0}}>
                    <button className="hover-scale" onClick={()=>goToSaved(s)} style={{...C.btnGhost,padding:'6px 12px',fontSize:12}}>🗺️ Ver</button>
                    <button className="hover-scale" onClick={()=>deleteSaved(s.id)} style={{...C.btnGhost,padding:'6px 12px',fontSize:12,color:'#f87171',borderColor:'rgba(239,68,68,.3)'}}>🗑</button>
                  </div>
                </div>
              ))}
            </div>
