    ...
    if (existing.length > 0) {
      ...
      ;[r] = await sql`
        UPDATE wellness_logs SET
          ...
          horas_sueno       = ${clampHours(b.horas_sueno)},
          tqr               = ${clamp(b.tqr)},
      ...
      ;[r] = await sql`
        INSERT INTO wellness_logs(
          ...
          dolor_zona, dolor_descripcion, dolor_eva, horas_sueno, tqr, recovery, entrena_grupo, fue_gimnasio, grupos_musculares, club_id
        ) VALUES(
          ...
          ${clampHours(b.horas_sueno)}, ${clamp(b.tqr)}, ${clamp(b.recovery)},
    ...
    if (String(e).includes('dolor_descripcion')) {
      if (existing.length > 0) {
        ...
        ;[r] = await sql`
          INSERT INTO wellness_logs(
            jugador_id, fecha, fatiga, calidad_sueno, dolor_muscular, nivel_estres, estado_animo,
            dolor_zona, dolor_eva, horas_sueno, tqr, recovery, entrena_grupo, fue_gimnasio, grupos_musculares, club_id
          ) VALUES(
            ${jugador_id}, ${fecha}, ${clamp(b.fatiga)}, ${clamp(b.calidad_sueno)},
            ${clamp(b.dolor_muscular)}, ${clamp(b.nivel_estres)}, ${clamp(b.estado_animo)},
            ${b.dolor_zona || null}, ${clamp0(b.dolor_eva)}, ${clampHours(b.horas_sueno)}, ${clamp(b.tqr)}, ${clamp(b.recovery)},
            ${b.entrena_grupo ?? true}, ${b.fue_gimnasio ?? false},
            ${b.grupos_musculares || null}, ${clubId}
          )
          RETURNING id, fecha::text`
...
export async function DELETE(req: NextRequest) {
  ...
  if (id) {
    await sql`DELETE FROM wellness_logs WHERE id = ${id} AND jugador_id IN (SELECT id FROM jugadores WHERE club_id = ${s.clubId})`
    return NextResponse.json({ success: true })
  }

  if (fecha) {
    if (jugador_id) {
      await sql`DELETE FROM wellness_logs WHERE jugador_id = ${jugador_id} AND fecha = ${fecha} AND jugador_id IN (SELECT id FROM jugadores WHERE club_id = ${s.clubId})`
    } else {
      await sql`DELETE FROM wellness_logs WHERE fecha = ${fecha} AND jugador_id IN (SELECT id FROM jugadores WHERE club_id = ${s.clubId})`
    }
    return NextResponse.json({ success: true })
  }
