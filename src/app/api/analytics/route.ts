...
export async function GET(req: NextRequest) {
  ...
  // FIX DE FECHA: Usamos el final del día para no dejar nada afuera
  const hastaFinal = new Date(hasta + 'T23:59:59.999Z').toISOString()
...
// DELETE: Para limpiar los datos de Analytics cuando se borra el calendario
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (clubId) {
      // Borramos logs de wellness y de entrenamiento de los jugadores del club
      await sql`DELETE FROM wellness_logs WHERE club_id = ${clubId}`;
