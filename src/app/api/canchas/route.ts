...
export async function GET() {
  try {
    ...
    const rows = clubId
      ...
      : await sql`SELECT id, nombre, direccion, lat::text, lng::text, largo_m::text, ancho_m::text, area_m2::text, tipo_cancha, superficie, notas, osm_id::text, created_at::text FROM canchas WHERE admin_id=${userId} ORDER BY created_at DESC`
