...
function parseRawRows(raw: any[][]): Record<string, any>[] {
  ...
  return (raw.slice(1) as any[][])
    ...
    .map(row => {
      ...
      if (['team', 'average', 'promedio', 'total', 'equipo', 'media', 'max', 'máximo'].some(k => nl.includes(k))) return null
...
function parsePdfFromText(rawText: string): Record<string, any>[] {
  ...
  for (const line of lines) {
    ...
    if (['team', 'average', 'promedio', 'total', 'equipo', 'media', 'max', 'máximo'].some(k => nl.includes(k))) continue
...
function parsePdfBlobColumnar(lines: string[]): Record<string, any>[] | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (['promedio','moyenne','average','media','total'].some(k => normStr(s).includes(k))) { blobSectionStart = i; break }
...
export async function POST(req: NextRequest) {
  try {
    ...
    let parsedRows: any[] = []
    if (rows && Array.isArray(rows)) parsedRows = parseRawRows(rows)
