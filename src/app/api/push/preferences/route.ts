...
export async function GET(req: NextRequest) {
  ...
  let rows = await sql`
    SELECT * FROM notification_preferences WHERE usuario_id = ${session.userId}
  `.catch(() => [])
  ...
  const subCount = await sql`
    SELECT COUNT(*)::int AS count FROM push_subscriptions WHERE usuario_id = ${session.userId}
  `.catch(() => [{ count: 0 }])
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    try { await sql`ALTER TABLE notification_preferences ADD COLUMN IF NOT EXISTS alerta_wellness_reminder BOOLEAN DEFAULT true` } catch {}
    try { await sql`ALTER TABLE notification_preferences ADD COLUMN IF NOT EXISTS alerta_sesion_manana BOOLEAN DEFAULT true` } catch {}
    try { await sql`ALTER TABLE notification_preferences ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()` } catch {}
