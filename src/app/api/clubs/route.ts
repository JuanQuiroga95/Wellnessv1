...
export async function GET(req: NextRequest) {
  ...
  const clubs = await sql`
    SELECT c.id, c.nombre, c.logo_url, c.created_at::text,
...
export async function POST(req: NextRequest) {
  const s = await getSessionFromRequest(req)
  if (!s || !isMaster(s)) return NextResponse.json({error:'No autorizado'},{status:403})
  const { nombre, logo_url } = await req.json()
  if (!nombre) return NextResponse.json({error:'Nombre requerido'},{status:400})
  const sql = getDb()
  const [club] = await sql`INSERT INTO clubs(nombre,logo_url) VALUES(${nombre},${logo_url||null}) RETURNING id,nombre,logo_url`
...
export async function PATCH(req: NextRequest) {
  const s = await getSessionFromRequest(req)
  if (!s || !isMaster(s)) return NextResponse.json({error:'No autorizado'},{status:403})
  const { id, nombre, logo_url } = await req.json()
  if (!id) return NextResponse.json({error:'id requerido'},{status:400})
  const sql = getDb()
  await sql`UPDATE clubs SET nombre=COALESCE(${nombre??null},nombre), logo_url=COALESCE(${logo_url??null},logo_url) WHERE id=${id}`
