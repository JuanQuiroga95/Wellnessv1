...
export async function GET(req: NextRequest) {
  try {
    ...
    // FIX APLICADO: Ahora incluye hasta el último milisegundo del día elegido
    const hastaInc = (() => {
...
// FIX APLICADO: Agregamos el método DELETE para borrado total
export async function DELETE(req: NextRequest) {
  try {
    ...
    await sql.begin(async (sql) => {
      // 1. Borramos los logs de RPE asociados a los jugadores del club
      await sql`
      ...
      `
      // 2. Borramos las sesiones planificadas
      await sql`
        DELETE FROM sesiones_plan 
        WHERE club_id = ${clubId} 
        AND fecha >= ${desde}::date AND fecha <= ${hasta}::date
      `
    })
