  ...
  if (club_ids !== undefined && Array.isArray(club_ids)) {
    try {
      ...
      if (currentCid && !club_ids.includes(currentCid)) {
        ...
        await sql`UPDATE usuarios SET club_id=${currentCid} WHERE id=${id} AND rol IN ('admin', 'master_admin')`
      } else if (!currentCid && club_ids.length > 0) {
        await sql`UPDATE usuarios SET club_id=${club_ids[0]} WHERE id=${id} AND rol IN ('admin', 'master_admin')`
      } else if (club_ids.length === 0) {
        await sql`UPDATE usuarios SET club_id=NULL WHERE id=${id} AND rol IN ('admin', 'master_admin')`
      }
    } catch { /* graceful fallback */ }
  } else if (club_id !== undefined) {
    await sql`UPDATE usuarios SET club_id=${club_id} WHERE id=${id} AND rol IN ('admin', 'master_admin')`
    if (club_id) {
      try { await sql`INSERT INTO admin_clubs(admin_id, club_id) VALUES(${id}, ${club_id}) ON CONFLICT DO NOTHING` } catch {}
    }
  }

  if (activo !== undefined) await sql`UPDATE usuarios SET activo=${activo} WHERE id=${id} AND rol IN ('admin', 'master_admin')`
  if (password) {
    const h = await bcrypt.hash(password, 12)
    await sql`UPDATE usuarios SET password_hash=${h}, password_plain=${password} WHERE id=${id} AND rol IN ('admin', 'master_admin')`
  }
