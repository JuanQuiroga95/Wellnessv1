...
export const SCHEMA_STATEMENTS = [
  ...
  // Variables simples: rango de peso ideal (nutricionista) y nacionalidad
  ...
  `ALTER TABLE jugadores ADD COLUMN IF NOT EXISTS nacionalidad VARCHAR(100)`,
  ...
  `CREATE TABLE IF NOT EXISTS metricas_tacticas (
    ...
    jugador_id INTEGER REFERENCES jugadores(id) ON DELETE CASCADE,
    sesion_id INTEGER REFERENCES sesiones_plan(id) ON DELETE CASCADE,
    goles INTEGER DEFAULT 0,
    asistencias INTEGER DEFAULT 0,
    perdidas INTEGER DEFAULT 0,
    coordenadas_perdida JSONB DEFAULT '[]',
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(jugador_id, sesion_id)
  )`,
  // ── Missing column migrations ─────────────────────────────────────────────
  `ALTER TABLE wellness_logs ADD COLUMN IF NOT EXISTS dolor_descripcion TEXT`,
  `ALTER TABLE wellness_logs ADD COLUMN IF NOT EXISTS horas_sueno NUMERIC(4,2)`,
  `ALTER TABLE entrenamiento_logs ADD COLUMN IF NOT EXISTS rpe_gimnasio INTEGER`,
  ...
  // ── Módulo Mi Rutina de Fuerza (10 Mandamientos) ──────────────────────────
  `CREATE TABLE IF NOT EXISTS fuerza_mandamientos (
    id SERIAL PRIMARY KEY,
    numero INTEGER NOT NULL UNIQUE,
    nombre VARCHAR(255) NOT NULL
  )`,
  `CREATE TABLE IF NOT EXISTS fuerza_ejercicios (
    id SERIAL PRIMARY KEY,
    club_id INTEGER REFERENCES clubs(id) ON DELETE CASCADE,
    nombre VARCHAR(255) NOT NULL,
    url_video TEXT,
  ...
  `CREATE TABLE IF NOT EXISTS fuerza_rutinas (
    id SERIAL PRIMARY KEY,
